...
# -*- coding: utf-8 -*-
#
# You can run this ansible helper as:
#               python runme.py
#            or with "update" task:
#               python runme.py update
# or by bundled Swift application in .dmg image [https://en.wikipedia.org/wiki/Apple_Disk_Image]
...
try:
    ...
    if manual and update_task is None:
    ...
    if need_run_ansible:
        if update_task:
            print(colored_green("\nRun update") + " ansible task now...")
            print("Remote server will be updated to latest " + colored_green("master") + " commit")

            ansible_playbook = [
                "ansible-playbook", "-i", "{0}/ansible_config/server".format(current_path),
                "{0}/ansible_config/update.yml".format(current_path)
            ]
        else:
            print(colored_green("\nRun ansible now..."))
        ...
        import subprocess
        subprocess.call(ansible_playbook)